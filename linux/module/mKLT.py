...
def pos(s, mop_score=False, remove_tag=[], c_tag=[], _opt = '-tip1+sw', _in = 'sample.in', _out = 'sample.out', thread=True):
    try:
        c_tag = c_tags
        ...
        command = [module_path+"/kma",_opt,_in,_out]
        ...
        if mop_score:
            c = Counter(poses)
            poses = [(mop[0],mop[1],c[mop]) for mop in poses]
